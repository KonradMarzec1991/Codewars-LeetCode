...
SELECT s.department_id AS id, SUM(price)
FROM sales s
...
SELECT member_id AS id, m.name AS name, m.email AS email, SUM(price) as total_spending
FROM sales s
INNER JOIN members m ON s.member_id = m.id
INNER JOIN products p ON s.product_id = p.id
WHERE department_id IN (SELECT id FROM dep_aggs)
